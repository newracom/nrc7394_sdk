...
static int I2CReadReg8(SC16IS750_t *dev, int reg)
{
	uint8_t output;

	nrc_i2c_start(dev->i2c);
	nrc_i2c_writebyte(dev->i2c, (dev->device_address_i2c << 1) | 0x00); // Write mode to set register
	nrc_i2c_writebyte(dev->i2c, reg);
	nrc_i2c_stop(dev->i2c);
	_delay_ms(1);

	nrc_i2c_start(dev->i2c); // Repeated start
	nrc_i2c_writebyte(dev->i2c, (dev->device_address_i2c << 1) | 0x01); // Read mode
	nrc_i2c_readbyte(dev->i2c, &output, false); // false for NACK (last byte)
	nrc_i2c_stop(dev->i2c);
	_delay_ms(1);
...
static int I2CWriteReg8(SC16IS750_t *dev, int reg, int data)
{
	nrc_i2c_start(dev->i2c);
	nrc_i2c_writebyte(dev->i2c, (dev->device_address_i2c << 1) | 0x00); // Write mode
	nrc_i2c_writebyte(dev->i2c, reg);   // Register address
	nrc_i2c_writebyte(dev->i2c, data);  // Data to write
	nrc_i2c_stop(dev->i2c);
	_delay_ms(1);
...
static void I2CSetup(SC16IS750_t *dev)
{
	if (nrc_i2c_init(dev->i2c) != NRC_SUCCESS) {
		printf("nrc_i2c_init is failed\n");
	}
	if (nrc_i2c_enable(dev->i2c, true) != NRC_SUCCESS) {
		printf("nrc_i2c_enable is failed\n");
	}
	printf("I2CSetup is success\n");
}

static int SPIReadReg8(SC16IS750_t *dev, int reg)
{
	uint8_t output;
	nrc_spi_readbyte_value(dev->spi, reg, &output);

	return output;
}

static int SPIWriteReg8(SC16IS750_t *dev, int reg, int data)
{
	nrc_spi_writebyte_value(dev->spi, reg, data);
	return data;
}

static void SPISetup(SC16IS750_t *dev)
{
	if (nrc_spi_master_init(dev->spi) != NRC_SUCCESS) {
		printf("nrc_spi_master_init is failed\n");
	}
	if (nrc_spi_enable(dev->spi, true) != NRC_SUCCESS) {
		printf("nrc_spi_enable is failed\n");
	}
	printf("SPISetup is success\n");
}
...
{
	uint8_t result = 0;
	//printf("ReadRegister channel=%d reg_addr=%x ",channel, (reg_addr<<3 | channel<<1));
	if ( dev->protocol == SC16IS750_PROTOCOL_I2C ) {	// register read operation via I2C
		result = I2CReadReg8(dev, (reg_addr<<3 | channel<<1));
		//printf("result=0x%x\n",result);
	} else if (dev->protocol == SC16IS750_PROTOCOL_SPI) {	//register read operation via SPI
		result = SPIReadReg8(dev, 0x80|(reg_addr<<3 | channel<<1));
		//printf("result=0x%x\n",result);
	}
	return result;
...
void SC16IS750_WriteRegister(SC16IS750_t * dev, uint8_t channel, uint8_t reg_addr, uint8_t val)
{
	...
	if ( dev->protocol == SC16IS750_PROTOCOL_I2C ) {	// register read operation via I2C
		I2CWriteReg8(dev, (reg_addr<<3 | channel<<1), val);
	} else {
		SPIWriteReg8(dev, (reg_addr<<3 | channel<<1), val);
	}
	return ;
}
...
int16_t SC16IS750_SetBaudrate(SC16IS750_t * dev, uint8_t channel, uint32_t baudrate) //return error of baudrate parts per thousand
{
	...
	if (divisor2 > divisor1) {
		printf("This baudrate (%ld) is not support\n",baudrate);
		return 0;
	}
	...
	if (error != 0) {
		printf("Warning:baudrate=%ld actual_baudrate=%ld\n", baudrate, actual_baudrate);
	}
