...
/*************************************************************/
void
save_tcpinfo(struct iperf3_stream *sp, struct iperf3_interval_results *irp)
{
#if defined(linux) || defined(__FreeBSD__)
    socklen_t tcp_info_length = sizeof(struct tcp_info);

    if (getsockopt(sp->socket, IPPROTO_TCP, TCP_INFO, (void *)&irp->tcpInfo, &tcp_info_length) < 0)
	iperf3_err(sp->test, "getsockopt - %s", strerror(errno));
...
/*************************************************************/
long
get_total_retransmits(struct iperf3_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
 ...
 */
long
get_snd_cwnd(struct iperf3_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
...
#ifdef notdef
/*************************************************************/
//print_tcpinfo(struct iperf3_interval_results *r)
void
print_tcpinfo(struct iperf3_test *test)
{
#if defined(linux)
...
/*************************************************************/
void
build_tcpinfo_message(struct iperf3_interval_results *r, char *message)
{
#if defined(linux)
