 ...
 */
int
iperf3_udp_recv(struct iperf3_stream *sp)
{
    int       r;
	...
	//iperf3_err(sp->test, "OUT OF ORDER - incoming packet = %d and received packet = %d AND SP = %d", pcount, sp->packet_count, sp->socket);
    }

    /* jitter measurement */
    iperf3_gettimeofday(&arrival_time);
 ...
 */
int
iperf3_udp_send(struct iperf3_stream *sp)
{
    int r;
    uint32_t  sec, usec, pcount;
    int       size = sp->settings->blksize;
 ...
 */
int
iperf3_udp_accept(struct iperf3_test *test)
{
 ...
 */
int
iperf3_udp_listen(struct iperf3_test *test)
{
    int s;
 ...
 */
int
iperf3_udp_connect(struct iperf3_test *test)
{
    int s, buf, sz;
#ifdef SO_RCVTIMEO
    struct timeval tv;
 ...
 */
int
iperf3_udp_init(struct iperf3_test *test)
{
    return 0;
}
