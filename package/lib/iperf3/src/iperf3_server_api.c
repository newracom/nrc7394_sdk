 ...
 */
/* iperf3_server_api.c: Functions to be used by an iperf server
*/
...
int
iperf3_server_listen(struct iperf3_test *test)
{
    retry:
    ...
    */
    ...
    if (test->protocol->id == Ptcp) {
	    ...
	    if (!test->json_output)
		iperf3_printf(test, report_window, ubuf);
        } else {
	    if (!test->json_output)
		iperf3_printf(test, "%s", report_autotune);
        }
    }
    */
    if (!test->json_output)
...
int
iperf3_accept(struct iperf3_test *test)
{
    int s;
	...
	if (iperf3_set_send_state(test, PARAM_EXCHANGE) != 0)
            return -1;
        if (iperf3_exchange_parameters(test) < 0)
            return -1;
	if (test->server_affinity != -1)
	    if (iperf3_setaffinity(test->server_affinity) != 0)
		return -1;
        if (test->on_connect)
...
/**************************************************************************/
int
iperf3_handle_message_server(struct iperf3_test *test)
{
    int rval;
    ...
    if ((rval = Nread(test->ctrl_sck, (char*) &test->state, sizeof(signed char), Ptcp)) <= 0) {
        if (rval == 0) {
	    iperf3_err(test, "the client has unexpectedly closed the connection");
            i_errno = IECTRLCLOSE;
            test->state = IPERF3_DONE;
            return 0;
        } else {
    ...
    switch(test->state) {
	    ...
	    if (iperf3_set_send_state(test, EXCHANGE_RESULTS) != 0)
                return -1;
            if (iperf3_exchange_results(test) < 0)
                return -1;
	    if (iperf3_set_send_state(test, DISPLAY_RESULTS) != 0)
                return -1;
	    ...
	    iperf3_err(test, "the client has terminated");
            SLIST_FOREACH(sp, &test->streams, streams) {
                FD_CLR(sp->socket, &test->read_set);
                FD_CLR(sp->socket, &test->write_set);
                close(sp->socket);
...
void
iperf3_test_reset(struct iperf3_test *test)
{
    struct iperf3_stream *sp;
...
static int
create_server_timers(struct iperf3_test * test)
{
    struct timeval now;
    TimerClientData cd;
...
static int
create_server_omit_timer(struct iperf3_test * test)
{
    struct timeval now;
    TimerClientData cd;
...
static void
cleanup_server(struct iperf3_test *test)
{
...
int
iperf3_run_server(struct iperf3_test *test)
{
    ...
    if (test->json_output) {
		...
		iperf3_printf(test, "%s\n", version);
		iperf3_printf(test, "%s", "");
		//fflush(stdout);
    }

    // Open socket and listen
    if (iperf3_server_listen(test) < 0) {
        return -1;
    }
    ...
    while (test->state != IPERF3_DONE) {
	    ...
	    if (result > 0) {
            ...
            if (FD_ISSET(test->ctrl_sck, &read_set)) {
                if ((ret = iperf3_handle_message_server(test)) < 0) {
		            cleanup_server(test);
                    return -1;
		        }
            ...
            if (test->state == CREATE_STREAMS) {
                ...
                if (streams_accepted == test->num_streams) {
                    ...
                    test->prot_listener = -1;
		            if (iperf3_set_send_state(test, TEST_START) != 0) {
			            cleanup_server(test);
                        return -1;
		            }
		            ...
		            if (test->reverse)
			            if (iperf3_create_send_timers(test) < 0) {
			                cleanup_server(test);
			                return -1;
			            }
		            if (iperf3_set_send_state(test, TEST_RUNNING) != 0) {
			            cleanup_server(test);
                        return -1;
		            }
                }
		...
		/* If select timeout (result == 0) and we did supply a timeout, run timers */
