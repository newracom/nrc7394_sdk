...
#if MAKE_COOKIE_USE_STATIC_HOSTNAME
    const char* hostname_static = "make_cookie_iperf3_client";
    strcpy(hostname, hostname_static);
#else
    (void) gethostbyname(hostname);
#endif
    iperf3_gettimeofday(&tv);
 ...
 * in iperf3_tcp_accept is not considered an error.
 */
...
void cpu_util(double pcpu[3])
{
    ...
    if (pcpu == NULL) {
        iperf3_gettimeofday(&last);
        return;
    }

    iperf3_gettimeofday(&temp);
...
**
** Sample call:
**   j = iperf3_json_printf("foo: %b  bar: %d  bletch: %f  eep: %s", b, i, f, s);
**
...
*/
cJSON *iperf3_json_printf(const char *format, ...)
{
    cJSON* o;
    va_list argp;
    const char *cp;
...
void iperf3_dump_fdset(FILE *fp, char *str, int nfds, fd_set *fds)
{
    int fd;
    int comma;
