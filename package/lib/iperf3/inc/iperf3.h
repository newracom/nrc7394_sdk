...
#define strdup		iperf3_strdup
extern char	*iperf3_strdup(const char *);


typedef uint64_t iperf3_size_t;

struct iperf3_interval_results
{
    iperf3_size_t bytes_transferred; /* bytes transfered in this interval */
    ...
    int interval_sacks;
    int snd_cwnd;
    TAILQ_ENTRY(iperf3_interval_results) irlistentries;
    void     *custom_data;
};

struct iperf3_stream_result
{
    iperf3_size_t bytes_received;
    iperf3_size_t bytes_sent;
    iperf3_size_t bytes_received_this_interval;
    iperf3_size_t bytes_sent_this_interval;
    ...
    TAILQ_HEAD(irlisthead, iperf3_interval_results) interval_results;
    void     *data;
};

#define COOKIE_SIZE 37		/* size of an ascii uuid */
struct iperf3_settings
{
    ...
    iperf3_size_t bytes;             /* number of bytes to send */
...
struct iperf3_stream
{
    ...
    int       socket;
    int       id;
	/* XXX: is settings just a pointer to the same struct in iperf3_test? if not,
		should it be? */
    struct iperf3_settings *settings;	/* pointer to structure settings */

    /* non configurable members */
    struct iperf3_stream_result *result;	/* structure pointer to result */
    Timer     *send_timer;
    int       green_light;
    ...
    int       (*rcv) (struct iperf3_stream * stream);
    int       (*snd) (struct iperf3_stream * stream);

    /* chained send/receive routines for -F mode */
    int       (*rcv2) (struct iperf3_stream * stream);
    int       (*snd2) (struct iperf3_stream * stream);

//    struct iperf3_stream *next;
    SLIST_ENTRY(iperf3_stream) streams;
...
struct protocol {
    int       id;
    char      *name;
    int       (*_accept)(struct iperf3_test *);
    int       (*_listen)(struct iperf3_test *);
    int       (*_connect)(struct iperf3_test *);
    int       (*_send)(struct iperf3_stream *);
    int       (*_recv)(struct iperf3_stream *);
    int       (*_init)(struct iperf3_test *);
    SLIST_ENTRY(protocol) protocols;
};

struct iperf3_textline {
    char *line;
    TAILQ_ENTRY(iperf3_textline) textlineentries;
};
...
struct iperf3_test
{
    ...
    void      (*stats_callback) (struct iperf3_test *);
    void      (*reporter_callback) (struct iperf3_test *);
    Timer     *omit_timer;
    Timer     *timer;
    int        done;
    Timer     *stats_timer;
...
//    struct iperf3_stream *streams;               /* pointer to list of struct stream */
    SLIST_HEAD(slisthead, iperf3_stream) streams;
    struct iperf3_settings *settings;

    SLIST_HEAD(plisthead, protocol) protocols;

    /* callback functions */
    void      (*on_new_stream)(struct iperf3_stream *);
    void      (*on_test_start)(struct iperf3_test *);
    void      (*on_connect)(struct iperf3_test *);
    void      (*on_test_finish)(struct iperf3_test *);
    ...
    TAILQ_HEAD(iperf3_textlisthead, iperf3_textline) server_output_list;
...
/* Use static hostname for cookie */
#define MAKE_COOKIE_USE_STATIC_HOSTNAME 1

extern int iperf3_gettimeofday(struct timeval *tv);
