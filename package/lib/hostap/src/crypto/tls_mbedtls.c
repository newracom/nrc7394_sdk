					 ...
					 struct wpabuf **appl_data)
{
	...
	while (tls->ssl.state != MBEDTLS_SSL_HANDSHAKE_OVER) {
		...
		if (ret < 0 || 
			((conn->tls_io_data.out_data && tls->ssl.conf->endpoint == MBEDTLS_SSL_IS_CLIENT) &&
			 (tls->ssl.state == MBEDTLS_SSL_SERVER_HELLO || tls->ssl.state == MBEDTLS_SSL_SERVER_CHANGE_CIPHER_SPEC)))
			break;
	}
...
{
	return os_snprintf(buf, buf_len, "mbedtls");
}
