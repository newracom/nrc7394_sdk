...
enum nl80211_chan_width convert2nlwidth(int width)
{
	switch (width) {
	case 8:
				   ...
				   const struct hostapd_freq_params *freq)
{
	...
	if (freq->vht_enabled || (freq->he_enabled && !is_24ghz)) {
		...
		wpa_printf(MSG_DEBUG, "  * freq_fractional=%d",
		...
		if (nla_put_u32(msg, NL80211_ATTR_CHANNEL_WIDTH, cw) ||
		    ...
		    (freq->center_freq2 &&
		     nla_put_u32(msg, NL80211_ATTR_CENTER_FREQ2,
				 freq->center_freq2)))
			return -ENOBUFS;
		if (nla_put_u32(msg, NL80211_ATTR_CENTER_FREQ_FRACTION,
				freq->freq_fractional))
			return -ENOBUFS;
	} else {
			       ...
			       struct hostapd_freq_params *freq, int set_chan)
{
	...
	wpa_printf(MSG_DEBUG,
		   "nl80211: Set freq %d.%d (ht_enabled=%d, vht_enabled=%d, he_enabled=%d, s1g_enabled=%d, bandwidth=%d MHz, cf1=%d MHz, cf2=%d MHz)",
		   freq->freq, freq->freq_fractional, freq->ht_enabled, freq->vht_enabled, freq->he_enabled, freq->s1g_enabled,
				  ...
				  size_t csa_offs_len)
{
	...
	wpa_printf(MSG_MSGDUMP, "nl80211: CMD_FRAME freq=%u bw=%u wait=%u no_cck=%d "
		   "no_ack=%d offchanok=%d",
		   freq, convert2nlwidth(bss->bandwidth), wait, no_cck, no_ack, offchanok);
	wpa_hexdump(MSG_MSGDUMP, "CMD_FRAME", buf, buf_len);

	if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_FRAME)) ||
	    (freq && nla_put_u32(msg, NL80211_ATTR_WIPHY_FREQ, freq)) ||
	    (bss->bandwidth && nla_put_u32(msg, NL80211_ATTR_CHANNEL_WIDTH,
			convert2nlwidth(bss->bandwidth))) ||		
