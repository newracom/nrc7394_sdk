			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		wpa_msg(wpa_s, MSG_INFO, "supp_port: authorised (state %u)",state);
#endif /* IEEE8021X_EAPOL */
		wpa_s->after_wps = 0;
...
#if defined(NRC_WPA_SUPP)
			if (wpa_driver_ps_get_recovered() != 0)
...
static bool ibss_mesh_is_80mhz_avail(int channel, struct hostapd_hw_modes *mode)
{
	...
	for (i = channel; i < channel + 16; i += 4) {
		...
		chan = hw_get_channel_chan(mode, i, NULL, NULL);
		if (!chan ||
...
#endif /* CONFIG_HE_OVERRIDES */
	if (hostapd_set_freq_params(&vht_freq, mode->mode, freq->freq, 0,
...
void wpa_supplicant_apply_s1g_overrides(
	struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid,
	struct wpa_driver_associate_params *params)
{
	...
	if (params->s1gcaps || params->s1g_caps_mask)
		return;


}
#endif /* CONFIG_S1G_OVERRIDES */
