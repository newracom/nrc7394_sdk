				     ...
				     int only_first_ssid, int debug_print)
{
	...
	/* NOTE: AP(s) can be ignored during scanning if they are in the ignore list.
	   This feature may trigger wrong connections with APs that have low RSSI. */
					   ...
					   struct wpa_bss *selected)
{
	...
	wpa_msg(wpa_s, MSG_INFO, "Considering within-ESS reassociation");
	wpa_msg(wpa_s, MSG_INFO, "Current BSS: " MACSTR
		" freq=%d level=%d snr=%d est_throughput=%u",
		MAC2STR(current_bss->bssid),
		current_bss->freq, current_bss->level,
		current_bss->snr, current_bss->est_throughput);
	wpa_msg(wpa_s, MSG_INFO, "Selected BSS: " MACSTR
		" freq=%d level=%d snr=%d est_throughput=%u",
	...
	if (wpa_s->current_ssid->bssid_set &&
	    ...
	    0) {
		wpa_msg(wpa_s, MSG_INFO, "Allow reassociation - selected BSS "
			"has preferred BSSID");
		return 1;
	}
	...
	if (cur_level < 0 && cur_level > selected->level + to_5ghz * 2 &&
	    sel_est < cur_est * 1.2) {
		wpa_msg(wpa_s, MSG_INFO, "Skip roam - Current BSS has better "
			"signal level");
		return 0;
	}
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		if ((ssid != wpa_s->current_ssid || selected != wpa_s->current_bss) &&
#else
		if (ssid != wpa_s->current_ssid &&
#endif
			wpa_s->wpa_state >= WPA_AUTHENTICATING) {
