...
#if defined(CONFIG_FIXED_PMK)
static int wpa_config_parse_pmk(const struct parse_data *data,
				struct wpa_ssid *ssid, int line,
				const char *value)
{
	extern u8 use_inserted_pmk;
	extern u8 inserted_pmk[PMK_LEN];

	if (hexstr2bin(value, inserted_pmk, PMK_LEN) ||
	    value[PMK_LEN * 2] != '\0') {
		wpa_printf(MSG_ERROR, "Line %d: Invalid PMK '%s'.",
			   line, value);
		os_memset(inserted_pmk, 0, PMK_LEN);
		return -1;
	}

	use_inserted_pmk = 1;
	wpa_hexdump_key(MSG_MSGDUMP, "PMK", inserted_pmk, PMK_LEN);
	return 0;
}


static char * wpa_config_write_pmk(const struct parse_data *data,
				   struct wpa_ssid *ssid)
{
	extern u8 use_inserted_pmk;
	extern u8 inserted_pmk[PMK_LEN];
	...
	if (use_inserted_pmk)
		return wpa_config_write_string_hex(inserted_pmk, PMK_LEN);

	return NULL;
}
#endif /* CONFIG_FIXED_PMK */
