...
void sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
	...
	if (data->auth.auth_type == WLAN_AUTH_SAE) {
		...
		if (res != 1)
			return;
		wpa_msg(wpa_s, MSG_INFO, "SME: setting PMK, BSSID:"MACSTR"",
...
#ifdef CONFIG_S1G_OVERRIDES
	struct ieee80211_s1g_capabilities s1gcaps;
	struct ieee80211_s1g_capabilities s1gcaps_mask;
#endif /* CONFIG_VHT_OVERRIDES */
	os_memset(&params, 0, sizeof(params));
...
#ifdef CONFIG_S1G_OVERRIDES
	os_memset(&s1gcaps, 0, sizeof(s1gcaps));
	os_memset(&s1gcaps_mask, 0, sizeof(s1gcaps_mask));
	params.s1gcaps = &s1gcaps;
	params.s1gcaps_mask = &s1gcaps_mask;
	wpa_supplicant_apply_s1g_overrides(wpa_s, wpa_s->current_ssid, &params);
#endif /* CONFIG_VHT_OVERRIDES */
