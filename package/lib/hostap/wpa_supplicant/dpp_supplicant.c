...
int wpas_dpp_auth_init(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	if (tcp)
		return dpp_tcp_init(wpa_s->dpp, auth, &ipaddr, tcp_port,
				    ...
				    /* SoftAP uses wpa_s context */
				    wpa_s->conf->dpp_name,
				    wpa_s->ap_iface? DPP_NETROLE_AP : DPP_NETROLE_STA,
#else
					      ...
					      struct dpp_config_obj *conf)
{
	...
	if (auth->net_access_key) {
		...
		if (wpa_s->ap_iface && wpa_s->ap_iface->bss[0]) {
			if (ssid->dpp_pfs < 2) {
				struct hostapd_bss_config *conf = wpa_s->ap_iface->bss[0]->conf;
				conf->dpp_netaccesskey = wpabuf_dup(auth->net_access_key);
			}
		}
#endif
#endif /* defined(CONFIG_AP) || defined(INCLUDE_SOFT_AP) */
...
#if defined(NRC_WPA_SUPP)
	/* Remove existing DPP network configuration if any */
	ssid = wpa_s->conf->ssid;
	while (ssid) {
		if ((ssid->key_mgmt & WPA_KEY_MGMT_DPP) &&
			ssid->dpp_connector && ssid->dpp_csign &&
			ssid->dpp_netaccesskey) {
			break;
		}
		ssid = ssid->next;
...
#if defined(NRC_WPA_SUPP)
		/* STA automatically try to connect if configuration was successful */
		if (status == DPP_STATUS_OK) {
			if (wpa_s->conf->dpp_config_processing == 2)
#if defined(CONFIG_AP) || defined(INCLUDE_SOFT_AP)
...
#if defined(NRC_WPA_SUPP)
static void wpas_dpp_send_peer_disc_resp(struct wpa_supplicant *wpa_s,
					    ...
					    enum dpp_status_error status)
{
	...
	len = 5 + 5 + 4 + os_strlen(dpp_connector);
#ifdef CONFIG_DPP2
	len += 5;
#endif /* CONFIG_DPP2 */
...
skip_status:
	...
	if (status == DPP_STATUS_OK &&
		...
		connector = dpp_corrupt_connector_signature(dpp_connector);
		if (!connector) {
			wpabuf_free(msg);
			return;
		}
	...
	if (status == DPP_STATUS_OK) {
		wpabuf_put_le16(msg, DPP_ATTR_CONNECTOR);
		wpabuf_put_le16(msg, os_strlen(dpp_connector));
		wpabuf_put_str(msg, dpp_connector);
	}
...
skip_connector:
	...
	if (DPP_VERSION > 1) {
		...
		conn_ver = dpp_get_connector_version(dpp_connector);
...
#if defined(CONFIG_AP) || defined(INCLUDE_SOFT_AP)
static void wpas_dpp_rx_peer_disc_req(struct wpa_supplicant *wpa_s,
				       const u8 *src,
				       const u8 *buf, size_t len,
				       unsigned int freq)
{
	...
	wpa_printf(MSG_DEBUG, "DPP: Peer Discovery Request from " MACSTR,
		   MAC2STR(src));
	if (!wpa_s->ap_iface || !wpa_s->ap_iface->bss[0]) {
		wpa_printf(MSG_DEBUG, "DPP: No AP interface");
		return;
	}

	ssid = wpa_s->conf->ssid;
	while (ssid) {
		if (ssid->dpp_connector && ssid->dpp_csign &&
			ssid->dpp_netaccesskey) {
			break;
		}
		ssid = ssid->next;
	...
	if (res != DPP_STATUS_OK) {
		...
		wpas_dpp_send_peer_disc_resp(wpa_s, NULL, src, freq, trans_id[0],
						res);
		return;
	}
	...
	if (intro.peer_version && intro.peer_version >= 2) {
		...
		if (attr_version != intro.peer_version) {
			...
			wpas_dpp_send_peer_disc_resp(wpa_s, NULL, src, freq,
							trans_id[0],
							DPP_STATUS_NO_MATCH);
			return;
		}
	...
	if (!expire || (os_time_t) ssid->dpp_netaccesskey_expiry < expire)
		expire = ssid->dpp_netaccesskey_expiry;
	if (expire)
		expiration = expire - now.sec;
	else
		expiration = 0;

	if (wpa_auth_pmksa_add3(wpa_s->ap_iface->bss[0]->wpa_auth, src,
				intro.pmk, intro.pmk_len, intro.pmkid,
				expiration,
	...
	wpas_dpp_send_peer_disc_resp(wpa_s, ssid->dpp_connector, src, freq, trans_id[0],
					DPP_STATUS_OK);
}
#endif /* defined(CONFIG_AP) || defined(INCLUDE_SOFT_AP) */
#endif /* NRC_WPA_SUPP */
			...
			const u8 *buf, size_t len, unsigned int freq)
{
	...
	case DPP_PA_PEER_DISCOVERY_REQ:
		wpas_dpp_rx_peer_disc_req(wpa_s, src, buf, len, freq);
		break;
#endif /* defined(CONFIG_AP) || defined(INCLUDE_SOFT_AP) */
...
int wpas_dpp_configurator_sign(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	wpas_dpp_set_testing_options(wpa_s, auth);
#if defined(NRC_WPA_SUPP)
	/* configurator netrole is determined by the command */
	if (dpp_set_configurator(auth, cmd) == 0) {
		int ap = 0;
		if (os_strstr(cmd, " conf=ap-")) {
			ap = 1;
		}
		if (dpp_configurator_own_config(auth, curve, ap) == 0)
...
skip_proto_ver:
	...
	wait_time = wpa_s->max_remain_on_chan;
#if !defined(NRC_WPA_SUPP)
	/* We need more time to wait for peer discovery response */
	if (wait_time > 2000)
		wait_time = 2000;
#endif /* !defined(NRC_WPA_SUPP) */
	wpa_msg(wpa_s, MSG_INFO, DPP_EVENT_TX "dst=" MACSTR " freq=%u type=%d",
...
int wpas_dpp_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->dpp) {
		...
		/* STA try to connect to the created profile automatically */
...
#if defined(NRC_WPA_SUPP)
	/* DPP spec describes these perferred channels for 11ah chirping */
	int preferred_freq = 0;

	if (strncmp(wpa_s->conf->country, "US", 2) == 0)
		preferred_freq = 5240;
	else if (strncmp(wpa_s->conf->country, "AU", 2) == 0 ||
		strncmp(wpa_s->conf->country, "NZ", 2) == 0)
		preferred_freq = 5205;
	else if (strncmp(wpa_s->conf->country, "SG", 2) == 0 ||
		strncmp(wpa_s->conf->country, "EU", 2) == 0)
		preferred_freq = 5180;
	else
		return freqs;
...
int wpas_dpp_push_button(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	if (!wpa_s->dpp)
		return -1;
#if defined(NRC_WPA_SUPP)
	for (int i = 0; i < DPP_PB_INFO_COUNT; i++) {
		struct dpp_pb_info *info = &wpa_s->dpp_pb[i];
		/* Allow a new push button session to be established
		 * immediately without the successfully completed
		 * session triggering session overlap. */
