...
static int wpas_ctrl_set_blob(struct wpa_supplicant *wpa_s, char *pos)
{
	...
	*pos++ = '\0';
	...
	blob->data = os_malloc(len);
	if (blob->name == NULL || blob->data == NULL) {
		wpa_config_free_blob(blob);
		return -1;
	}

	os_memcpy(blob->data, pos, len);
#if 0	/* mbedtls not support bin2hexstr */
					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
		...
		wpa_s->conf->sae_pwe = atoi(value);
		if (wpa_s->conf->sae_pwe < 0 ||
			wpa_s->conf->sae_pwe > 2) {
			wpa_printf(MSG_ERROR,
				   "Invalid SAE PWE value %d",
				   wpa_s->conf->sae_pwe);
			return -1;
		}
