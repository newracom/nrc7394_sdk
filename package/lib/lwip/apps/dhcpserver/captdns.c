...
typedef struct __attribute__((packed))DnsLabel{
	uint8_t len;
	uint8_t data;
} DnsLabel;


typedef struct __attribute__((packed))DnsQuestionFooter{
	//before: label
	uint16_t type;
	uint16_t class;
} DnsQuestionFooter;
...
typedef struct __attribute__((packed))DnsResourceFooter{
	//before: label
	uint16_t type;
	uint16_t class;
	uint32_t ttl;
...
typedef struct __attribute__((packed))DnsUriHdr{
	uint16_t prio;
	uint16_t weight;
} DnsUriHdr;
...
//Receive a DNS packet and maybe send a response back
 void captdnsRecv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length) {
	...
	p+=sizeof(DnsHeader);
	LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("DNS packet: id 0x%X flags 0x%X rcode 0x%X qcnt %d ancnt %d\
	...
	for (i=0; i<my_ntohs(&hdr->qdcount); i++) {
		...
		LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("DNS: Q (type 0x%X class 0x%X) for %s\n",\
		...
		if (my_ntohs(&qf->type)==QTYPE_A) {
			...
			*rend++=ip4_addr1(ip_2_ip4(&info.ip));
			*rend++=ip4_addr2(ip_2_ip4(&info.ip));
			*rend++=ip4_addr3(ip_2_ip4(&info.ip));
			*rend++=ip4_addr4(ip_2_ip4(&info.ip));
			setn16(&rhdr->ancount, my_ntohs(&rhdr->ancount)+1);
			LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("Added A rec to resp. Resp len is %d\n", (rend-reply)));
			...
			*rend++=0;
			setn16(&rhdr->ancount, my_ntohs(&rhdr->ancount)+1);
			LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("Added NS rec to resp. Resp len is %d\n", (rend-reply)));
	...
	//Send the response
	sendto(sockFd,(uint8_t*)reply, rend-reply, 0, (const struct sockaddr *)premote_addr, sizeof(struct sockaddr_in));
}
...
static void captdnsTask(void *pvParameters) {
	...
	do {
		...
		if (sockFd==-1) {
			LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("captdns_task failed to create sock!\n"));
	...
	do {
		...
		if (ret!=0) {
			LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("captdns_task failed to bind sock!\n"));
			vTaskDelay(1000/portTICK_RATE_MS);
		}
	} while (ret!=0);

	LWIP_DEBUGF(DNS_DEBUG | LWIP_DBG_TRACE, ("CaptDNS inited.\n"));
	while(1) {
