...
#include "mz_crypt.h"
#include "mz_strm.h"
/* NRC SDK embedded port: mz_strm_zlib.c is always compiled in. */
#if defined(MZ_EMBEDDED) && !defined(HAVE_ZLIB)
#  define HAVE_ZLIB
#endif
#ifdef HAVE_BZIP2
        ...
        while ((err == MZ_OK) && (field_pos + 4 <= file_info->extrafield_size)) {
            ...
            /* Read UNIX1 extra field */
                ...
                if ((err == MZ_OK) && (linkname_size > 0)) {
                    linkname = (char *)mz_malloc(linkname_size);
                    if (linkname) {
...
static int32_t mz_zip_read_cd(void *handle) {
    ...
    if (err == MZ_OK) {
        ...
        if ((err == MZ_OK) && (comment_size > 0)) {
            zip->comment = (char *)mz_malloc(comment_size + 1);
            if (zip->comment) {
...
void *mz_zip_create(void) {
    mz_zip *zip = (mz_zip *)mz_calloc(1, sizeof(mz_zip));
    if (zip)
...
int32_t mz_zip_set_comment(void *handle, const char *comment) {
    ...
    if (comment_size > UINT16_MAX)
        return MZ_PARAM_ERROR;
    zip->comment = (char *)mz_calloc(comment_size + 1, sizeof(char));
    if (!zip->comment)
        return MZ_MEM_ERROR;
