...
static mz_alloc_malloc_cb g_mz_malloc_cb;
static mz_alloc_calloc_cb g_mz_calloc_cb;
static mz_alloc_free_cb   g_mz_free_cb;
static void              *g_mz_alloc_opaque;

void mz_set_allocator(mz_alloc_malloc_cb malloc_cb,
                      mz_alloc_calloc_cb calloc_cb,
                      mz_alloc_free_cb free_cb,
                      void *opaque) {
    g_mz_malloc_cb = malloc_cb;
    g_mz_calloc_cb = calloc_cb;
    g_mz_free_cb = free_cb;
    g_mz_alloc_opaque = opaque;
}

void *mz_malloc(size_t size) {
    if (g_mz_malloc_cb)
        return g_mz_malloc_cb(g_mz_alloc_opaque, size);
    return malloc(size);
}

void *mz_calloc(size_t items, size_t size) {
    if (g_mz_calloc_cb)
        return g_mz_calloc_cb(g_mz_alloc_opaque, items, size);
    return calloc(items, size);
}

void mz_free(void *ptr) {
    if (!ptr)
        return;
    if (g_mz_free_cb) {
        g_mz_free_cb(g_mz_alloc_opaque, ptr);
        return;
    }
    free(ptr);
}
...
void *mz_stream_raw_create(void) {
    mz_stream_raw *raw = (mz_stream_raw *)mz_calloc(1, sizeof(mz_stream_raw));
    if (raw)
