...
static voidpf mz_stream_zlib_zalloc(voidpf opaque, uInt items, uInt size) {
    MZ_UNUSED(opaque);
    return mz_calloc(items, size);
}

static void mz_stream_zlib_zfree(voidpf opaque, voidpf address) {
    MZ_UNUSED(opaque);
    mz_free(address);
}
...
int32_t mz_stream_zlib_open(void *stream, const char *path, int32_t mode) {
    ...
    zlib->zstream.zalloc = mz_stream_zlib_zalloc;
    zlib->zstream.zfree = mz_stream_zlib_zfree;
...
void *mz_stream_zlib_create(void) {
    mz_stream_zlib *zlib = zlib = (mz_stream_zlib *)mz_calloc(1, sizeof(mz_stream_zlib));
    if (zlib) {
